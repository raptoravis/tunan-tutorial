...
预期：
- 写入 `.tunan-workspace/req/REQ-002-voting-system.md`，`status: ready`
...
预期：写入 `prd/PRD-002-voting-system.md`，status=ready；推荐 `/tunan-story PRD-002`。
...
落盘 `plan/PLAN-001-create-poll.md`。
...
落盘 `testplan/TESTPLAN-001-create-poll.md`。
...
- 落盘 `pr/PR-001-create-poll.md`，`gh_pr: <num>`，`status: reviewing`
...
落盘 `req/REQ-003-vote-count-not-refresh.md`，`kind: bug`，`status: ready`。
...
req/
  ...
  REQ-003-vote-count-not-refresh.md  (done, kind:bug)
prd/
  PRD-002-...md                  (done)
  PRD-003-...md                  (done)
story/
