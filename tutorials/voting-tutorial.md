...
| **横切元工具** | tunan-prime / tunan-align / tunan-cp / tunan-parallel-pick / tunan-story-graph / tunan-retro / tunan-improve-arch / tunan-newrawreq / tunan-rawreq-organize / **tunan-pipeline** | 看现状 / 对齐 / 提交推送 / 并行拣选 / 依赖图 / 回顾 / 架构反思 / 对话起草 raw-req / raw-req 整理 / **一键全流程** |
...
> 不想自己写文件，但已经在对话里把需求 + 截图都聊清楚了？跑 `/tunan-newrawreq` —— 它会把对话里的需求陈述 + 所有粘贴的 asset（图 / 视频 / log）一步落成 `.tunan-workspace/raw-reqs/<current_sprint>/<slug>/` 自包含子目录，asset 语义化重命名、MD 同目录平级引用。kind=bug 时会先做复现完整性自检，缺复现路径会硬挡你补。产物可以直接 `/tunan-req <slug>/<slug>.md` 入池。
>
...
- claude 提 5 个 STORY 候选（标准切分）：
  1. STORY-001：创建投票（标题、选项、单/多选、截止时间）
  2. STORY-002：投一票
  3. STORY-003：看结果（计数、百分比）
  4. STORY-004：截止后只读
  5. STORY-005：每用户一票约束
- 内调 align：
  - Q1 切分粒度（★标准 5 个 / 粗 3 个 / 细 8 个）
  - Q2 哪些 P0（★STORY-001/002/003）
  - Q3 STORY-004/005 是否本轮做（★做）
...
预期：claude 按 blocked_by + 影响文件冲突分析，建议本轮 STORY-001 单跑（根节点）；004/005 解锁后可与 003 并行。
...
落盘 `<STORY-dir>/PLAN-001-create-poll.md`。
...
落盘 `<STORY-dir>/TESTPLAN-001-create-poll.md`。
...
- 落盘 `<STORY-dir>/PR-001-create-poll.md`，`gh_pr: <num>`，`status: reviewing`
...
落盘 `sprints/SPT-001/reqs/REQ-003-vote-count-not-refresh/REQ-003-vote-count-not-refresh.md`，`kind: bug`，`status: ready`。
...
> "issue REQ-003（投票后无法看到结果计数）出现了。我们的 rules / skills / workflow 中有什么可以改的，让这种问题不再发生？"
...
sprints/SPT-001/reqs/
  ...
  REQ-003-vote-count-not-refresh/                          (kind:bug)
    REQ-003-vote-count-not-refresh.md                      (done)
    PRD-003-.../
      PRD-003-...md                                        (done)
      STORY-006-.../                                       (done, bug)
