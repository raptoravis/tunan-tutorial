...
> 本教程**不**生成投票系统的真实代码。它把 tunan 工作流的每一步用一个具体例子（投票系统）走完，让你照着 sponsor 该说的话、claude 该有的响应，亲手把整套流程跑一遍。
>
> 教程分两轮：
>
> 1. **第一轮**：从对话生成 REQ → PRD → STORY → PLAN → TESTPLAN → worktree dev → PR 自驱动循环 → merge → verify
> 2. **第二轮**：预设一个 bug（"投票后无法看到结果计数"），走 issue→REQ→…→merge，并用 retro 真实修改某个 tunan-* skill
...
- 在仓库根跑 `./check-tunan-windows.ps1`（Windows）或 `./check-tunan.sh`（macOS/Linux）输出 **All checks passed**
  - 校验内容：前置 CLI（git / node / uv / gh / claude / ruflo）、3 个 MCP（memory / context7 / playwright）、tunan-* skill 集、`.tunan-workspace/{worktrees,retro,raws,sprints}` + `worktrees/.gitignore` + 带 `current_sprint` 的 `settings.md`、`raws/<current>/` 与 `sprints/<current>/reqs/`、`USER.md`、`tunan-tutorials/`、`gh` 已登录
  - 任一红色 `[ERROR]` 都先解决再继续
...
- 示例：`S01-RAW001-voting-system`、`S01-REQ001-voting-system`、`S01-STORY003-see-results`

目录布局（每个 raw / REQ / PRD / STORY 都是自包含子目录，目录名 = 同名 MD 文件名）：
...
├── raws/                             # raw 收件箱
│   └── S01/
│       └── S01-RAW001-voting-system/
│           ├── S01-RAW001-voting-system.md
│           └── <asset>.png           # 粘贴的截图/视频，语义化命名，同目录平级
└── sprints/
    └── S01/
        └── reqs/
            └── S01-REQ001-voting-system/
                ├── S01-REQ001-voting-system.md
                └── S01-PRD001-voting-system/
                    ├── S01-PRD001-voting-system.md
                    └── S01-STORY001-create-poll/
                        ├── S01-STORY001-create-poll.md
                        ├── S01-PLAN001.md
                        ├── S01-TESTPLAN001.md
                        └── S01-PR001.md
```
...
| 组            | skill                                                                 | 角色                                              |
|---------------|-----------------------------------------------------------------------|---------------------------------------------------|
| **主链路 6 站** | tunan-req / tunan-prd / tunan-story / tunan-plan / tunan-testplan / tunan-dev | 把 raw MD 文件逐站推到代码                       |
...
| **横切元工具** | tunan-prime / tunan-align / tunan-cp / tunan-parallel-pick / tunan-story-graph / tunan-retro / tunan-improve-arch / tunan-newraw / tunan-raw-check / tunan-dashboard / **tunan-pipeline** | 看现状 / 对齐 / 提交推送 / 并行拣选 / 依赖图 / 回顾 / 架构反思 / 对话起草 raw / 未入池 raw 盘点 / 池可视化看板 / **一键全流程** |
...
> **一键全流程**：信任默认时，先用 `/tunan-newraw` 在 `.tunan-workspace/raws/<current_sprint>/` 下落一份 raw，再 `/tunan-pipeline <raw.md>`，它会把下面的步骤 1-9 串起来，每个对齐点应用 ★ 默认。本教程仍按"分站手动"走，便于看清每环职责。

### skill / 开关覆盖速查

这张表是本教程的覆盖检查清单：**主线会实际跑常用路径；不适合在投票例子里真实演示的开关，也在这里说明用途和风险边界**。

| skill | 本教程覆盖方式 | 调用 / 开关说明 |
|---|---|---|
| `tunan-req` | 步骤 1、1b、10 实跑 | `/tunan-req <raw.md>` 默认形态，必须传 `.tunan-workspace/raws/<current_sprint>/` 下的 MD 文件路径；`--from-issue <#>` 从 GitHub issue 入池（issue 正文自动落档到 `raws/<current_sprint>/`）；`--from-conversation` **显式**从当前会话抽取（必须显式指定，不再缺省）；默认开启 research，`--no-research` 关闭调研；`--kind=feature\|bug\|chore` 指定类型；`--id=S<MM>-REQNNN` 指定 id；`--owner=<name>` 指定 owner。想"一路推到 verify" 走 `/tunan-pipeline <raw.md>`。 |
...
| `tunan-newraw` | 步骤 1 实跑 | `/tunan-newraw` 默认从当前对话抽取，落成自包含 raw 子目录 `.tunan-workspace/raws/<current_sprint>/S<MM>-RAW<NNN>-<slug>/`，内含 `<同名>.md` + 对话里粘贴的所有 asset（图 / 视频 / log），asset 语义化重命名、MD 同目录平级引用。`<slug>` 指定 slug 跳过自动命名；`--sprint=S<MM>` 指定 sprint；`--dry-run` 只看计划不动盘；`--align` 启用逐点对齐（默认快通路、不弹菜单）。kind=bug 且 `--align` 时会做复现完整性自检。产物可直接喂给 `/tunan-req` 或 `/tunan-pipeline`。 |
| `tunan-raw-check` | 步骤 1 预备说明 | `/tunan-raw-check` 扫 `raws/<current_sprint>/` 与 `sprints/<current_sprint>/reqs/S*-REQ*/` 做 set diff，列出尚未被任何 REQ 引用的 raw；`--all-sprints` 跨 sprint；`--sprint=S<MM>` 指定 sprint；`--show-linked` 连已入池的也列；`--json` 输出 JSON。纯只读，不动盘。 |
| `tunan-dashboard` | 进阶段落说明 | `/tunan-dashboard` 起本地 Python http 看板（默认 `port=37856`，自动开浏览器），7 列展示 raw→REQ→PRD→STORY→PLAN→TESTPLAN→PR，可在 UI 直接改 `assignee/owner/status/priority` 并自动追写 `actor_log`；`--port=<N>` 换端口；`--workspace=<path>` 指定 workspace；`--no-open` 不自动开浏览器；`--read-only` 只读模式；`--stop` 停掉看板。 |
| `tunan-pipeline` | 全景说明、调试备忘说明 | `/tunan-pipeline <raw.md>` 默认形态，从 raw MD 文件全链路；`--from-issue <#>` 从 issue；`--from-conversation` 显式从当前对话抽取；`<REQ-id>`/`<PRD-id>`/`<STORY-id>` 从已有条目续推；`--stop-at=plan` 到 PLAN 停；`--no-merge` 到 sponsor_wait 停；默认 H1 red-gate 与 H2 PR LGTM 预授权一路到底，`--need-auth` 才回到逐 gate 等待 sponsor。 |
...
### 步骤 1 · 起草 raw → 生成 REQ（缺省 --research）

最省事的方式：在对话里把需求说清楚（可以贴截图），然后让 `/tunan-newraw` 把它落成 raw 子目录：

```
sponsor> 我想做一个投票系统：可以投票去哪里旅游 / 中饭吃什么。
         （随便展开几句细节：截止时间？单选/多选？匿名/实名？想不清也没关系。）
sponsor> /tunan-newraw
```

预期：claude 把对话里的需求陈述 + 所有粘贴的 asset 一步落成自包含子目录 `.tunan-workspace/raws/S01/S01-RAW001-voting-system/`，内含 `S01-RAW001-voting-system.md`（H1 下标注 `> **类型**` / `> **创建者**`）+ 语义化重命名的 asset。缺省走快通路、不弹对齐菜单；要逐点确认传 `--align`。

> 想自己手写 raw 也行——按命名约定建子目录即可：
> ```
> sponsor> mkdir -p .tunan-workspace/raws/S01/S01-RAW001-voting-system
> sponsor> # 在 S01-RAW001-voting-system/ 下写 S01-RAW001-voting-system.md
> ```

raw 落盘后入池：

```
sponsor> /tunan-req .tunan-workspace/raws/S01/S01-RAW001-voting-system/S01-RAW001-voting-system.md
```

> 真的只想从对话原话抽取、不落 raw 草稿？给 `/tunan-req` 显式传 `--from-conversation`；默认不再静默 fallback 到对话。
>
> 写了一批 raw、想知道哪些还没入 REQ 池？跑 `/tunan-raw-check`——它对 `raws/` 与 `sprints/.../reqs/` 做 set diff，列出未被任何 REQ 引用的 raw。

预期：
- claude **开始**播报"将走 1) 抽取 2) research 3) top3 4) 入池"
- 抽取意图后，**用 WebSearch / context7 跑社区调研**：找类似投票工具的常见复杂度（截止时间、多选 vs 单选、防刷票、匿名/实名、结果可见性）
...
预期：
- 写入 `.tunan-workspace/sprints/S01/reqs/S01-REQ001-voting-system/S01-REQ001-voting-system.md`，`status: ready`（sprint 由 `settings.md.current_sprint` 决定）
- 结束播报：推荐 `/tunan-prd S01-REQ001` / `/tunan-cp` / `/tunan-prime`

### 步骤 1b · （可选）从 issue 生成 REQ

如果你不想自己写 raw，可以先在 GitHub 开 issue（issue 正文会自动落档一份到 `.tunan-workspace/raws/<current_sprint>/`）：
...
预期：写入 `<REQ-dir>/S01-PRD001-voting-system/S01-PRD001-voting-system.md`，status=ready；推荐 `/tunan-story S01-PRD001`。
...
- claude 提 5 个 STORY 候选（标准切分）：
  1. S01-STORY001：创建投票（标题、选项、单/多选、截止时间）
  2. S01-STORY002：投一票
  3. S01-STORY003：看结果（计数、百分比）
  4. S01-STORY004：截止后只读
  5. S01-STORY005：每用户一票约束
- 内调 align：
  - Q1 切分粒度（★标准 5 个 / 粗 3 个 / 细 8 个）
  - Q2 哪些 P0（★STORY001/002/003）
  - Q3 STORY004/005 是否本轮做（★做）
...
- STORY003 blocked_by STORY002
- STORY004/005 blocked_by STORY002
...
预期：claude 按 blocked_by + 影响文件冲突分析，建议本轮 S01-STORY001 单跑（根节点）；STORY004/005 解锁后可与 STORY003 并行。
...
- align：
  - Q1 框架选型（★Next.js + SQLite / Express+React / FastAPI+React）
...
- Persona：novice 创建投票 / power-user 复用模板 / adversarial 标题超长 / 选项 0 个 / 截止时间在过去
...
预期一次调用：
1. fetch 评论 → 无
2. reviewer 跑一轮 → 发现"缺少输入校验" → 写 PR 评论
3. coder 修 → push → reviewer 再跑 → pass
4. tester 跑 → smoke pass / 1 个 adversarial 失败"标题 1024 字符崩溃" → 写评论
...
合并后，sponsor 自己用一下投票系统，发现：**投了一票后页面没立即显示新计数**（需手动刷新）。开 GitHub issue：

```
sponsor> gh issue create --title "投票后无法看到结果计数" --body "..."
```
...
落盘 `sprints/S01/reqs/S01-REQ003-vote-count-not-refresh/S01-REQ003-vote-count-not-refresh.md`，`kind: bug`，`status: ready`。
...
> bug 流程的 PRD/STORY 通常短小（"Given 用户提交投票 When 提交成功 Then 计数立即更新无需刷新"），PLAN 里直接引用诊断报告里的"根因 + 复现测试"。
...
> "issue S01-REQ003（投票后无法看到结果计数）出现了。我们的 rules / skills / workflow 中有什么可以改的，让这种问题不再发生？"
...
- 写 `retro/2026-MM-DD-vote-count-not-refresh.md`（含触发 issue id / 改了哪些 skill / 决议来源）
...
预期：claude 起一个本地 Python http 服务（默认端口 37856）并开浏览器，7 列从左到右展示 raw→REQ→PRD→STORY→PLAN→TESTPLAN→PR；卡片可按 creator/owner/assignee/status/sprint 过滤；可在 UI 直接改 `assignee/owner/status/priority` 并自动追写 `actor_log`。`--stop` 停掉。
...
sprints/S01/reqs/
  S01-REQ001-voting-system/
    S01-REQ001-voting-system.md                            (done)
    S01-PRD001-.../
      S01-PRD001-...md                                     (done)
      S01-STORY001..005-.../
        S01-STORYNNN-*.md / S01-PLANNNN.md / S01-TESTPLANNNN.md / S01-PRNNN.md (全 done)
  S01-REQ003-vote-count-not-refresh/                       (kind:bug)
    S01-REQ003-vote-count-not-refresh.md                   (done)
    S01-PRD003-.../
      S01-PRD003-...md                                     (done)
      S01-STORY006-.../                                    (done, bug)
        S01-STORY006-*.md / S01-PLAN/TESTPLAN/PR-*.md
retro/
  2026-MM-DD-vote-count-not-refresh.md
```
...
- 想一键跑全流程而不是分站 → 先 `/tunan-newraw` 落一份 raw，再 `/tunan-pipeline <raw.md>`（参 skill 全景表）
- 不确定哪些 raw 还没入 REQ 池 → `/tunan-raw-check`
